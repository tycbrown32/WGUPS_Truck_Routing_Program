...
class Truck:
    ...
    def package_delivery(self):
        ...
        while len(self.packages) > 0:
            ...
            package_hash.search(remove_package).delivery_time = self.truck_time.strftime('%H:%M')
...
# Function for determine package status a specified time ---------------------------------------------------------------
...
req_time_str = input("Enter time to check package delivery status (H:M, 24-hour): ")
req_time = time_correction(req_time_str)

package_status(req_time, package_hash, package_hash_key)

print('\nTotal distance travelled is', float("%0.2f" % (truck1.distance_travelled + truck2.distance_travelled)),
      'miles.\n')
